		     ...
		     TString era)
{
  ...
  for (auto& it : histos)   { it.second->Sumw2(); it.second->SetDirectory(0); }
  ...
  for(Int_t fileIter = 0; fileIter < nFiles; fileIter++){
   ...
   if(channelSelection==11 || channelSelection==1100) 
   {
	...
	for(Int_t elIter = 0; elIter < nEl; elIter++)
      ...
      deposit =  elePFPhoIso_p->at(elIter)+elePFNeuIso_p->at(elIter)-0.5*elePFPUIso_p->at(elIter);
      db_isolation = (elePFChIso_p->at(elIter) + TMath::Max (0.0, deposit )) / elePt_p->at(elIter);

      
//      bool relIso=(elePFChIso_p->at(elIter)-TMath::Max(elePFPhoIso_p->at(elIter)+elePFNeuIso_p->at(elIter)-elePFPUIso_p->at(elIter)))/elePt_p->at(elIter);
//      bool relIso=(elePFChIso_p->at(elIter)-TMath::Max((elePFPhoIso_p->at(elIter)+elePFNeuIso_p->at(elIter)-elePFPUIso_p->at(elIter)),0.))/elePt_p->at(elIter);
//	    int LooseId = eleIDLoose_p->at(elIter);
//	    int MediumId = eleIDMedium_p->at(elIter);
//	    int VetoId = eleIDVeto_p->at(elIter);
    	    
	    //save electron if good
	    TLorentzVector p4(0,0,0,0);
	    p4.SetPtEtaPhiM(elePt_p->at(elIter),eleEta_p->at(elIter),elePhi_p->at(elIter), 0.0510);
	    if(passMediumId && db_isolation < 0.12)     
	      {
		mediumElectrons.push_back( p4 );
		elCharge.push_back(eleCharge_p->at(elIter));
	      }
        if (passMediumId && db_isolation > 0.4)
        {
	...
	if(channelSelection==11)
	  {
	    ...
	    if(vetoElectrons.size()) continue;    //no extra electrons
	    if(chargeSelection!=0)
	      {
...
//check for extra muons in the event (veto id is used)
  ...
  goodLeptons = (tightMuons.size() > 0) ?  tightMuons : mediumElectrons;    
